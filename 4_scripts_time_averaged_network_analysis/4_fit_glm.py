...
def fit_glm_and_do_stats(target):
    ...
    copes = model.copes
    pvalues = [
        do_stats(design, data, model, contrast_idx=0),
...
if do_aec:
    target = np.load("data/glm_aec.npy")
    copes, pvalues, mean = fit_glm_and_do_stats(target)
    np.save("data/glm_aec_copes.npy", copes)
    np.save("data/glm_aec_pvalues.npy", pvalues)
    np.save("data/glm_aec_mean.npy", mean)

if do_mean_aec:
    target = np.load("data/glm_mean_aec.npy")
    copes, pvalues, mean = fit_glm_and_do_stats(target)
    np.save("data/glm_mean_aec_copes.npy", copes)
