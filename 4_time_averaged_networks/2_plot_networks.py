...
if plot_mean_coh_vs_pow_vs_age:
    ...
    for P, C, eP, eC in zip(P_bands, C_bands, eP_bands, eC_bands):
        for p, c, ep, ec, g in zip(P, C, eP, eC, groups):
            ax.errorbar(p, c, xerr=ep, yerr=ec, c=cmap(g / groups.max()), lw=3)
    for P, C, m, l in zip(P_bands, C_bands, markers, labels):
        pc = ax.scatter(P, C, c=groups, marker=m, label=l, s=100)
...
if plot_psd_vs_age:
    ...
    # Calculate the average PSD for each group
    P = []
    for i in range(len(start_ages) - 1):
        keep = np.logical_and(age >= start_ages[i], age < start_ages[i + 1])
        P.append(np.mean(p[keep], axis=0))
    P = np.array(P)
    ...
    for band, color in zip(freq_bands, band_colors):
        ax.axvspan(band[0], band[1], color=color, alpha=0.3)
    ax.set_xlim(f[0], 45)
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(
        cm.ScalarMappable(norm=colors.Normalize(vmin=age.min(), vmax=age.max()),
        cmap=cmap),
        cax=cax,
        orientation="vertical",
    )
    cax.set_ylabel("Age (years)", fontsize=16)
    plt.tight_layout()
    filename = "plots/psd_vs_age.png"
