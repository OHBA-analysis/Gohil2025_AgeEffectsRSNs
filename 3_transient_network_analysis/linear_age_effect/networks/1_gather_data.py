...
# Load summary stats and transition probability matrices
...
def get_targets(id):
    ...
    mc = np.mean(c, axis=-1)
    c = c[:, m, n]
    return P, p, c, mc, tp[i], fo[i], lt[i], intv[i], sr[i]
...
for _, row in csv.iterrows():
    ...
    if Path(preproc_file).exists():
        ...
        # Get data
        P, p, c, mc, tp_i, fo_i, lt_i, intv_i, sr_i = get_targets(id)
        if len(p) == 0:
...
np.save("data/trans_prob.npy", tp_)
